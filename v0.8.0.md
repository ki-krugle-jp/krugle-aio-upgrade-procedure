...
## Notes
shasta initや、shastaのモデルダウンロードが失敗（下記のようなエラー）するような場合、/root/.shasta/shasta.confのhf_から始まる文字列が、
/root/krugle-aio-installer/shasta-linux/shasta.confに記載されている文字列と同一か確認してください。
もし、同一でない場合は、/root/krugle-aio-installer/shasta-linux/shasta.confに
記載されているhf_から始まる文字列で/root/.shasta/shasta.confを更新してください。
```
Installing KrugleAI models... -Error downloading md5.json
